...
with DAG(
    ...
    def start_dwh_session():
        ...
        # Unquoted name: PG folds to etl.dwh_assignsessionid (CREATE PROCEDURE without quotes)
        cursor.execute(
            "CALL etl.dwh_AssignSessionID(null::bigint, null::bigint, null::timestamp)"
        )
        ...
        for row in cursor:
            logging.info(
                "dwh session id = %s Count =%s, date =%s", row[0], row[1], row[2]
            )
        ...
        if v_count > 0:
            ...
            logging.info("Empty dwh id variable %d", session_row_id)
        ...
        if v_count > 0:
            ...
            cursor.execute(
                f"""CALL mq."sp_SaveSessionState"(NULL::bigint, {v_dwh_session_id}::bigint, {v_count}::bigint, 1::smallint, null::smallint, '{v_session_date}'::timestamp without time zone, null::varchar(4000))"""
            )
            dwh_session_id = 0
            for row in cursor:
                logging.info("session id = %s", row[0])
    ...
    def finish_dwh_session():
        ...
        cursor.execute(
            f"""CALL mq."sp_SaveSessionState"({v_session_id}::bigint, NULL::bigint, NULL::bigint, 1::smallint, 2::smallint, NULL::timestamp without time zone, null::varchar(4000))"""
        )
        cursor.close()
        src_conn.close()
        ...
        v_dwh_session_id = Variable.get("dwh_session_id", default=-1)
        cursor.execute(
            "CALL etl.dwh_SaveSessionState(%s::bigint, null::smallint, 4::smallint, null::varchar);",
            (v_dwh_session_id,),
        )
        cursor.close()
        src_conn.close()
