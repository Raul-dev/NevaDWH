...
BEGIN
  ...
  IF par_dwh_session_id IS NULL OR par_dwh_session_id = -1 THEN
    SELECT  min(dwh_session_id) into par_dwh_session_id FROM etl.dwh_session WHERE (par_dwh_session_id IS NULL OR par_dwh_session_id != -1) AND dwh_session_state_id = 2;

    RAISE NOTICE 'N1 par_dwh_session_id  %', par_dwh_session_id;

    IF NOT par_dwh_session_id IS NULL THEN
      SELECT COALESCE(create_session,now()) INTO par_create_session FROM etl.dwh_session WHERE dwh_session_id = par_dwh_session_id; 
      SELECT COALESCE(SUM(row_count),0) INTO par_rowcount
      FROM etl.dwh_processing_details WHERE dwh_session_id = par_dwh_session_id
      GROUP BY dwh_session_id;
      RAISE NOTICE 'N2 par_dwh_session_id  %', par_dwh_session_id;
      RETURN;
    END IF;

    call etl.dwh_SaveSessionState ( par_dwh_session_id::bigint , 1::smallint, 1::smallint, null::varchar(4000) );
  END IF;
  
  IF var_RowCount > 0 THEN
    call etl.dwh_SaveSessionState (par_dwh_session_id, 2, null,null);
  END IF;
--DIM_Валюты
  ...
  IF var_rowcount > 0 THEN
    INSERT INTO etl.dwh_processing_details( dwh_session_id, schema_name, table_name, row_count)
...
--FACT_Продажи
  ...
  IF par_rowcount > 0 THEN
    ...
    CALL etl.dwh_SaveSessionState (par_dwh_session_id::bigint, 1::smallint, 2::smallint, null) ;
  END IF;
