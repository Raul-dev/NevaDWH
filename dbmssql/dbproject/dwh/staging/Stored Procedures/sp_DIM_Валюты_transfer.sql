...
BEGIN
  ...
  SET @ProcedureName = '[staging].[sp_DIM_Валюты_transfer]'
  SET @ProcedureParams =
...
BEGIN TRY
  ...
  FROM [$(LinkSRVOds)].[$(ods)].[odins].[DIM_Валюты_history] tmp
...
BEGIN CATCH
  ...
  SELECT [SessionId] = @session_id,
    [SessionStateId] = 3,
    [ErrorMessage] = 'ETL transfer [odins_DIM_Валюты]. Error: ' +@ErrorMessage
