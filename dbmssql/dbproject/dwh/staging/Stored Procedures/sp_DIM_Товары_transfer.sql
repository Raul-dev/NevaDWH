CREATE PROCEDURE [staging].[sp_DIM_Товары_transfer]
...
BEGIN
  ...
  SET @ProcedureName = '[staging].[sp_DIM_Товары_transfer]'
  SET @ProcedureParams =
...
BEGIN TRY
  ...
  SELECT @LastTargetID = MAX(id) FROM [target].[DIM_Товары]
...
BEGIN CATCH
  ...
  SELECT [SessionId] = @session_id,
    [SessionStateId] = 3,
    [ErrorMessage] = 'ETL transfer [odins_DIM_Товары]. Error: ' +@ErrorMessage
