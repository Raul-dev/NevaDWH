CREATE TABLE [odins].[DIM_Клиенты_buffer] (
    ...
    [dt_update]   datetime2(4)   NOT NULL CONSTRAINT DF_odins_DIM_Клиенты_buffer_dt_update_DEFAULT DEFAULT (DATEFROMPARTS(1900, 01, 01)),
    [RefID]      AS ([dbo].[fn_GetRef]([msg],'CatalogObject.Клиенты')),
    CONSTRAINT [PK_odins_DIM_Клиенты_buffer] PRIMARY KEY CLUSTERED
    (
        [buffer_id] ASC
