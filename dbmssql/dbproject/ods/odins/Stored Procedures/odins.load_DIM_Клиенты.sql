...
SELECT * FROM [odins].[DIM_КлиентыBuffer]
SELECT * FROM [odins].[DIM_Клиенты]
...
FROM [odins].[DIM_КлиентыBuffer] D
EXEC [odins].[load_DIM_Клиенты]
...
BEGIN
  ...
  SET @ProcedureName = '[odins].[load_DIM_Клиенты]'
  SET @ProcedureParams =
...
BEGIN TRANSACTION
  ...
  IF ISNULL(@BufferId, 0) <= 0
    ...
    FROM [odins].[DIM_КлиентыBuffer] b 
  ...
  BEGIN
    ...
    WHERE l.[MessageTypeId] = 2 AND NOT EXISTS(SELECT 1 FROM [mq].[FileQueue] f WHERE f.[MessageKey] = 'CatalogObject.Клиенты' AND f.[MessageId] = l.[MessageId] AND f.[CreatedAt] = b.[CreatedAt]);

    DECLARE @FileQueueID bigint, @res int
    SELECT @FileQueueID = MIN([FileQueueId]) FROM [mq].[FileQueue] f WHERE f.[MessageKey] = 'CatalogObject.Клиенты' AND [StateId] in (1,3)
    COMMIT TRANSACTION
  ...
  ;WITH XMLNAMESPACES (DEFAULT 'http://v8.1c.ru/8.1/data/enterprise/current-config', 'http://www.w3.org/2001/XMLSchema-instance' as xsi)
  INSERT [staging].[DIM_Клиенты]([NKey],   [RefID],  [DeletionMark],  [Code],  [Description],  [Контакт], [UpdatedAt])
  SELECT
  ...
  FROM [odins].[DIM_КлиентыBuffer] AS b
  INNER JOIN @LockedListUniq l ON l.[BufferId] = b.[BufferId]
  CROSS APPLY b.[MessageBody].nodes('/Data/Реквизиты/CatalogObject.Клиенты') AS X(C);

  IF @Debug = 1
    SELECT [staging_DIM_Клиенты] = 'staging_DIM_Клиенты', * FROM [staging].[DIM_Клиенты]

  EXEC [odins].[load_DIM_Клиенты_staging]

  -- Clear buffer table
  IF @BufferHistoryMode = 1 AND NOT EXISTS (SELECT 1 FROM [odins].[DIM_КлиентыBuffer] WHERE [IsError] = 1)
  BEGIN
    DELETE b
    FROM [odins].[DIM_КлиентыBuffer] b
  ...
  BEGIN
    ...
    IF @BufferHistoryMode >= 2 AND NOT EXISTS (SELECT 1 FROM [odins].[DIM_КлиентыBuffer] WHERE [IsError] = 1)
      DELETE b
      FROM [odins].[DIM_КлиентыBuffer] b
...
BEGIN CATCH
  ...
  SELECT
    ...
    [SessionStateId] = 3,
    [ErrorMessage] = 'Table [odins].[DIM_КлиентыBuffer]. Error: ' + @ErrorMessage
