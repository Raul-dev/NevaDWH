...
SELECT * FROM [odins].[DIM_ВалютыBuffer]
SELECT * FROM [odins].[DIM_Валюты]
...
FROM [odins].[DIM_ВалютыBuffer] D
EXEC [odins].[load_DIM_Валюты]
...
BEGIN
  ...
  SET @ProcedureName = '[odins].[load_DIM_Валюты]'
  SET @ProcedureParams =
...
BEGIN TRANSACTION
  ...
  BEGIN
    ...
    WHERE l.[MessageTypeId] = 2 AND NOT EXISTS(SELECT 1 FROM [mq].[FileQueue] f WHERE f.[MessageKey] = 'CatalogObject.Валюты' AND f.[MessageId] = l.[MessageId] AND f.[CreatedAt] = b.[CreatedAt]);

    DECLARE @FileQueueID bigint, @res int
    SELECT @FileQueueID = MIN([FileQueueId]) FROM [mq].[FileQueue] f WHERE f.[MessageKey] = 'CatalogObject.Валюты' AND [StateId] in (1,3)
    COMMIT TRANSACTION
  ...
  ;WITH XMLNAMESPACES (DEFAULT 'http://v8.1c.ru/8.1/data/enterprise/current-config', 'http://www.w3.org/2001/XMLSchema-instance' as xsi)
  INSERT [staging].[DIM_Валюты]([NKey],   [DIM_Валюты.Представления],  [RefID],  [DeletionMark],  [Code],  [Description],  [ЗагружаетсяИзИнтернета],  [НаименованиеПолное],  [Наценка],  [ОсновнаяВалюта],  [ПараметрыПрописи],  [ФормулаРасчетаКурса],  [СпособУстановкиКурса], [UpdatedAt])
  SELECT
  ...
  FROM [odins].[DIM_ВалютыBuffer] AS b
  INNER JOIN @LockedListUniq l ON l.[BufferId] = b.[BufferId]
  CROSS APPLY b.[MessageBody].nodes('/Data/Реквизиты/CatalogObject.Валюты') AS X(C);

  IF @Debug = 1
    SELECT [staging_DIM_Валюты] = 'staging_DIM_Валюты', * FROM [staging].[DIM_Валюты]

  EXEC [odins].[load_DIM_Валюты_staging]

  -- Clear buffer table
  IF @BufferHistoryMode = 1 AND NOT EXISTS (SELECT 1 FROM [odins].[DIM_ВалютыBuffer] WHERE [IsError] = 1)
  BEGIN
    DELETE b
    FROM [odins].[DIM_ВалютыBuffer] b
  ...
  BEGIN
    ...
    IF @BufferHistoryMode >= 2 AND NOT EXISTS (SELECT 1 FROM [odins].[DIM_ВалютыBuffer] WHERE [IsError] = 1)
      DELETE b
      FROM [odins].[DIM_ВалютыBuffer] b
...
BEGIN CATCH
  ...
  SELECT
    ...
    [SessionStateId] = 3,
    [ErrorMessage] = 'Table [odins].[DIM_ВалютыBuffer]. Error: ' + @ErrorMessage
