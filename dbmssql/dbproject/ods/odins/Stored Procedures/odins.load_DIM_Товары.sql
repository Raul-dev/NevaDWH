...
BEGIN TRANSACTION
    ...
    -- Clear buffer table
    IF @BufferHistoryMode = 1 AND NOT EXISTS (SELECT 1 FROM [odins].[DIM_Товары_buffer] WHERE [is_error] = 1)
    BEGIN
        DELETE b
        FROM [odins].[DIM_Товары_buffer] b
    ...
    BEGIN
        ...
        IF @BufferHistoryMode >= 2 AND NOT EXISTS (SELECT 1 FROM [odins].[DIM_Товары_buffer] WHERE [is_error] = 1)
            DELETE b
            FROM [odins].[DIM_Товары_buffer] b
...
BEGIN CATCH
    ...
    SELECT session_id = @err_session_id,
        [session_state_id] = 3,
        [error_message] = 'Table [odins].[DIM_Товары_buffer]. Error: ' +@ErrorMessage
