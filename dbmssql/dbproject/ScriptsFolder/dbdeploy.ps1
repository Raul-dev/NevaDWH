...
Import-Module -Name './cmdfunction.psm1' -Verbose  
#Get-ChildItem -Path '..\..\..\..\..\Deployment\dbmssql'
...
try{
	...
	#Get-ChildItem
    $IsVS2019only = $false
	$obj= Get-VsInstallation $IsVS2019only
	if($null -eq $obj) {
	...
	$msbuildLocation = Get-MsbuildLocation $IsVS2019only
	...
	do {
        ...
        if (!(Test-Path $sourceFile) -or !($PublishOnly) -or ($IsRebuild)) {
	        ...
	        IF($PublishOnly -eq $false) {
		        $res = DropDatabaseFromServer $TargetDBname $TargetServerName $SQLuser $SQLpwd
